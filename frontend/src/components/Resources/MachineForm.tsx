...
interface Props {
  onSave: (data: Omit<Machine, 'id' | 'status'>) => void;
  onCancel: () => void;
}

export default function MachineForm({ onSave, onCancel }: Props) {
  const [form, setForm] = useState({ machineCode: '', name: '', type: '' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.machineCode || !form.name || !form.type) { setError('All fields are required'); return; }
    setError('');
    onSave({ machineCode: form.machineCode, name: form.name, type: form.type });
  };
